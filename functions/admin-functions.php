...
function woo_get_fw_version( $url = '', $check_if_critical = false ) {
	...
	if( ! empty( $url ) ) {
    	...
    	$fw_url = 'https://wooframework.s3.amazonaws.com/latest/functions-changelog.txt';
    }
...
function woo_title ( $echo = true ) {
	...
	if ( is_tax( 'product_cat' ) ) {
	   ...
	   // $title = $term->name;
	    $title = get_term_meta( $term->term_id, 'details', true ); 
	    if(!$title)
			$title = apply_filters( 'woo_title', $raw_title, $sep, $raw_title );
 ...
 */
function woo_meta () {
	echo '<meta https-equiv="Content-Type" content="'. esc_attr( get_bloginfo( 'html_type' ) ) . '; charset=' . esc_attr( get_bloginfo( 'charset' ) ) . '" />' . "\n";
	...
	function woo_google_webfonts() {
		...
		if ( ! empty( $woo_options ) && ! empty( $google_fonts ) ) {
			...
			if ( 0 < count( $fonts_to_load ) ) {
				...
				$output .= "\n<!-- Google Webfonts -->\n";
				$output .= '<link href="https+'. ( is_ssl() ? 's' : '' ) .'://fonts.googleapis.com/css?family=' . $fonts .'" rel="stylesheet" type="text/css" />'."\n";
